...
object GlobalValueConfig {
    ...
    /** 保存配置到PlatformStorage */
    fun saveToPrefs(storage: PlatformStorage) {
        val json = GameJsonObject()
        json.put("first_prestige_days", Lifecycle.FIRST_PRESTIGE_DAYS)
        json.put("first_prestige_level", Lifecycle.FIRST_PRESTIGE_LEVEL)
        json.put("soul_power_base", Realm.SOUL_POWER_BASE.toInt())
        json.put("soul_power_exponent", Realm.SOUL_POWER_EXPONENT.toInt())
        json.put("base_gold_drop_mult", DropRate.BASE_GOLD_DROP_MULT.toInt())
        json.put("offline_gain_efficiency", Retention.OFFLINE_GAIN_EFFICIENCY.toInt())
        storage.putString(KEY_CONFIG_JSON, json.toJsonString())
        storage.apply()
    }
    
    /** 从PlatformStorage加载配置 */
    fun loadFromPrefs(storage: PlatformStorage) {
        val jsonStr = storage.getString(KEY_CONFIG_JSON, null)
        if (jsonStr != null) {
            val json = GameJsonObject.fromString(jsonStr)
        }
    }
    
    /** 重置为默认配置 */
    fun resetToDefaults(storage: PlatformStorage) {
        storage.remove(KEY_CONFIG_JSON)
        storage.apply()
    }
