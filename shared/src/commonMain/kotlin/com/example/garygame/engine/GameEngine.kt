...
object GameEngine {
    ...
    fun getCultivateCooldownRemaining(): Int {
        val elapsed = PlatformTime.currentTimeMillis() - lastCultivateTime
    ...
    fun doEnhanceBone(boneTypeOrdinal: Int): Boolean {
        ...
        val boneName = BoneType.entries[boneTypeOrdinal].displayName
        addLog("🔨 $boneName 强化成功！+${newLevel} → 词缀效果 ×${formatDouble(EnhancementData.enhanceEffect(newLevel), 1)}")
        notifyUI(); return true
    }
    ...
    fun doUpgradeSoulCore(backpackIndex: Int): Boolean {
        ...
        addLog("⬆ ${target.name} 升级成功! Lv.${newLevel} (效果×${formatDouble(mult, 2)})")
        s.save()
        notifyUI(); return true
    }
    ...
    fun limitedShopRemainSec(): Int {
        val elapsed = PlatformTime.currentTimeMillis() - state.limitedShopRefreshTime
    ...
    fun doDungeonFight(): Boolean {
        ...
        if (result.won) {
            ...
            s.lastDungeonTime = PlatformTime.currentTimeMillis()
    ...
    fun doPrestige(): Boolean {
        ...
        addLog("📈 永久: 全属性 ×${formatDouble(prestigeMultiplier(), 1)}")
        val newFeatures = checkForNewFeatures()
        if (newFeatures.isNotEmpty()) {
            pendingFeatureUnlocks.addAll(newFeatures)
            val names = newFeatures.joinToString("、") { "${it.icon} ${it.displayName}" }
    ...
    fun formatNum(n: Long): String = when {
        n >= 1_000_000_000_000L -> "${formatDouble(n / 1_000_000_000_000.0, 1)}兆"
        n >= 100_000_000L -> "${formatDouble(n / 100_000_000.0, 1)}亿"
        n >= 10_000L -> "${formatDouble(n / 10_000.0, 1)}万"
        else -> n.toString()
    }
